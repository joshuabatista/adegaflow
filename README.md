Sistema web completo para **gerenciamento de adegas**, focado em **frente de caixa (PDV)**, **controle de estoque**, **lançamento de vendas** e **análise financeira através de gráficos**.

O Adega Flow foi desenvolvido para otimizar a rotina de pequenas e médias adegas, trazendo organização, agilidade no atendimento e visão clara dos resultados financeiros.

www.adegaflow.com.br

## 🚀 Funcionalidades

- 🧾 **Frente de Caixa (PDV)**
  - Lançamento rápido de vendas
  - Controle de formas de pagamento
  - Registro detalhado por produto

- 📦 **Controle de Estoque**
  - Entrada e saída automática de produtos
  - Alerta de estoque baixo
  - Organização por categorias

- 💰 **Lançamento e Histórico de Vendas**
  - Registro diário de vendas
  - Consulta por período
  - Relatórios detalhados

- 📊 **Análise Financeira**
  - Gráficos de faturamento
  - Análise de desempenho por período
  - Visão clara de entradas e resultados

![image alt](https://github.com/joshuabatista/adegaflow/blob/main/Screenshot_1.png?raw=true)
